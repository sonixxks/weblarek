...
### Слой Презентера
Презентер реализован в файле src/main.ts как набор функций-обработчиков событий.

Основные обработчики событий: `catalog:changed`, `basket:changed`, `card:select`, `basket:open`, `order:submit`